...
char diff_patts[256] = "EXIST=ONLY_SRC,EXIST=ONLY_DST,TYPE=DIFFER,SIZE=DIFFER,UID=DIFFER,GID=DIFFER,PERM=DIFFER,MTIME=DIFFER";
...
struct dsync_options {
    struct dsync_disjunction *disc;      /* dsync_disjunction */
    int verbose;
    ...
    const char* key)
{
    ...
    if (dsync_option_need_compare(DCMPF_ACL)) {
        if (dsync_compare_acl(src_list, src_index, dst_list, dst_index)) { 
                                ...
                                const mfu_param_path* dest_path)
{
     ...
     * directories */ 
    dsync_sync_files(src_map, dst_map, src_path, dest_path,
    ...
    int is_src)
{
    ...
    *number = 0;
    ...
    strmap_foreach(map, node) {
        ...
        if (dsync_disjunction_match(options.disc, map, key, is_src)) {
            (*number)++;
    ...
    list_for_each_entry(conjunction,
                        &options.disc->conjunctions,
                        linkage) {
        if (is_src)
            MPI_Allreduce(&(conjunction->src_matched), &(conjunction->src_matched), 1,
                          MPI_UINT64_T, MPI_SUM, MPI_COMM_WORLD);
        else
            MPI_Allreduce(&(conjunction->dst_matched), &(conjunction->dst_matched), 1,
                          MPI_UINT64_T, MPI_SUM, MPI_COMM_WORLD);
    }
    return 0;
}
    ...
    strmap* dst_map)
{
    ...
    ret = dsync_output_flist_match(src_map, src_flist,
                                  new_flist, &src_matched, 1);
    assert(ret == 0);

    /* find matched file in dest map */
    uint64_t dst_matched = 0;
    ret = dsync_output_flist_match(dst_map, dst_flist,
    ...
    if (rank == 0) {
        ...
        dsync_disjunction_print(options.disc, 0,
                               strlen(DCMP_OUTPUT_PREFIX));

        if (options.disc->count > 1)
...
static int dsync_disjunction_parse(const char *disjunction_string)
{
    int ret = 0;
...
static void dsync_diff_patterns_parse(const char *option)
{
    char* disjunction;

    disjunction = MFU_STRDUP(option);

    dsync_disjunction_parse(disjunction);
    mfu_free(&disjunction);
}
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        switch (c) {
        case 'a':
            strcat(diff_patts, ",ACL=DIFFER");
            break;
        case 'c':
            strcat(diff_patts, ",CONTENT=DIFFER");
            break;
        case 'd':
            options.debug++;
            break;
    ...
    if (usage) {
        ...
        dsync_disjunction_free(options.disc);
        mfu_finalize();
        MPI_Finalize();
        return 1;
    }

    dsync_diff_patterns_parse(diff_patts);
    ...
    dsync_output_write(flist3, map1, flist4, map2);
